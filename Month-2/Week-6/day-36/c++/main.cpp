...
std::string toLCD(int n, int m) {
    const int SPACE_BETWEEN_DIGITS = 2;
    ...
    //lambda to get the width of digit    
    const auto widthOfDigit = [&](std::tuple<int, int, int> ds[DIGIT_STRUCTURE_SIZE]) { 
        int w = DIGIT_DEFAULT_WIDTH;
        if (std::get<0>(ds[0]) == ENUM_DIGIT_CHAR_STRUCTURE::IGNORE) {
            w -= 1;
        }
        if (std::get<1>(ds[0]) == ENUM_DIGIT_CHAR_STRUCTURE::IGNORE) {
            w -= 1;
        }
        if (std::get<2>(ds[0]) == ENUM_DIGIT_CHAR_STRUCTURE::IGNORE) {
            w -= 1;
        }
        return w;
    };  
    ...
    const auto writer = [&](int s, int x, int yBase, int y, int width, int m, char *canvas) {
        if (s != ENUM_DIGIT_CHAR_STRUCTURE::IGNORE) {
            for (int r = 0; r < m; r++) {
                canvas[y * width + x++] = charOfDigit(s, yBase);
            }
        }
        return x; 
    };
    ...
    for (int i = 0; i < sn.size(); i++) {        
        ...
        case '0':
            ...
            break;
        case '1':
            d = new std::tuple<int, int, int>[DIGIT_STRUCTURE_SIZE]{std::make_tuple(ENUM_DIGIT_CHAR_STRUCTURE::SIMPLE_SPACE, ENUM_DIGIT_CHAR_STRUCTURE::IGNORE, ENUM_DIGIT_CHAR_STRUCTURE::IGNORE), std::make_tuple(ENUM_DIGIT_CHAR_STRUCTURE::REPEATABLE_CHAR, ENUM_DIGIT_CHAR_STRUCTURE::IGNORE, ENUM_DIGIT_CHAR_STRUCTURE::IGNORE), std::make_tuple(ENUM_DIGIT_CHAR_STRUCTURE::SIMPLE_SPACE, ENUM_DIGIT_CHAR_STRUCTURE::IGNORE, ENUM_DIGIT_CHAR_STRUCTURE::IGNORE), std::make_tuple(ENUM_DIGIT_CHAR_STRUCTURE::REPEATABLE_CHAR, ENUM_DIGIT_CHAR_STRUCTURE::IGNORE, ENUM_DIGIT_CHAR_STRUCTURE::IGNORE), std::make_tuple(ENUM_DIGIT_CHAR_STRUCTURE::SIMPLE_SPACE, ENUM_DIGIT_CHAR_STRUCTURE::IGNORE, ENUM_DIGIT_CHAR_STRUCTURE::IGNORE)};
            break;                                         
        case '2':
            ...
            break;     
        case '3':
            d = new std::tuple<int, int, int>[DIGIT_STRUCTURE_SIZE]{std::make_tuple(ENUM_DIGIT_CHAR_STRUCTURE::REPEATABLE_CHAR, ENUM_DIGIT_CHAR_STRUCTURE::SIMPLE_SPACE, ENUM_DIGIT_CHAR_STRUCTURE::IGNORE), std::make_tuple(ENUM_DIGIT_CHAR_STRUCTURE::REPEATABLE_SPACE, ENUM_DIGIT_CHAR_STRUCTURE::REPEATABLE_CHAR, ENUM_DIGIT_CHAR_STRUCTURE::IGNORE), std::make_tuple(ENUM_DIGIT_CHAR_STRUCTURE::REPEATABLE_CHAR, ENUM_DIGIT_CHAR_STRUCTURE::SIMPLE_SPACE, ENUM_DIGIT_CHAR_STRUCTURE::IGNORE), std::make_tuple(ENUM_DIGIT_CHAR_STRUCTURE::REPEATABLE_SPACE, ENUM_DIGIT_CHAR_STRUCTURE::REPEATABLE_CHAR, ENUM_DIGIT_CHAR_STRUCTURE::IGNORE), std::make_tuple(ENUM_DIGIT_CHAR_STRUCTURE::REPEATABLE_CHAR, ENUM_DIGIT_CHAR_STRUCTURE::SIMPLE_SPACE, ENUM_DIGIT_CHAR_STRUCTURE::IGNORE)};
            break;
        case '4':
    ...
    for (auto &d : digits) {
        if (width > 0) {
            width += SPACE_BETWEEN_DIGITS;
        }
        width += widthOfDigit(d) + (m - 1);
    }    
    ...
    for (auto &p : digits) {
        ...
        for (int y = 0; y < DIGIT_STRUCTURE_SIZE; y++) {              
            auto a = std::get<0>(p[y]);
            auto b = std::get<1>(p[y]);
            auto c = std::get<2>(p[y]);
            for (int r = 0; r < (isEven(y) ? 1 : m); r++) {
                ...
                xAux = writer(c, xAux, y, yAux, width, multiplierOfDigit(c, y, m), canvas);
                yAux++;
            }            
        }
        xSpaceOffset += widthOfDigit(p) + (m - 1) + SPACE_BETWEEN_DIGITS;
    }    
...
|  |     |    |    |  | |  | |    |  | |
    ...
    std::string outputSize1 = toLCD(123456890, 1);
...
|   |       |     |     |  |  |  |  |     |  |  |
     --   --          --    --    --    --    -- )";    
    std::string outputSize2 = toLCD(123456890, 2);
    CHECK(expectedSize2.compare(outputSize2.substr(0, expectedSize2.size())) == 0);

    std::ofstream myfile;
    myfile.open ("sample_lcd_digits_1_to_4.txt");
...
|    |         |      |      |  |   |  |   |      |  |   |
    ...
    std::string outputSize3 = toLCD(123456890, 3);
...
|     |           |       |       |  |    |  |    |       |  |    |
       ----   ----            ----    ----    ----    ----    ----)";
    std::string outputSize4 = toLCD(123456890, 4);
